...
smaller final accuracy; plots are omitted for brevity). It is also not clear
whether the target should apply to all layers equally. For instance, in networks
with the (now outdated) $\text{tanh}$ activation function, the later layer's gradients
...
multiplication with a factor $< 1$ (as the tanh derivative never exceeds $1$ and
decreases to almost $0$ in both limits), so gradients become exponentially
