...
having to repeat themselves, exchanging visualisations and metrics with others and
reduces the friction between development and debugging.
...
also overloads arithmetic operators for automatically constructing the graph.
The graph is recreated for each propagation through the network and the user
never directly interacts with it.  This precludes some optimizations, but makes
dynamically changing networks easily achievable and the API more natural to
users.
...
\begin{itemize}
    \item
        ...
        \begin{lstlisting}[label={lst:whilepy-pt}, gobble=8]
        ...
        \begin{lstlisting}[label={lst:whilepy-tf}, gobble=8]
    ...
    \item
        ...
        language feature they know but must learn how to express many concepts
...
\begin{figure}
    \hypertarget{fig:popularity}{%
        \centering
        \includegraphics[max width=.8\textwidth]{gfx/diagrams/framework_popularity/popularity.pdf}
...
The software is structured into several subpackages of the
\texttt{ikkuna} package. Other packages and modules contain utilities
...
The \texttt{models} subpackage (see \cref{sec:pack-models}) contains a few
exemplary neural network definitions which are wired up with the library and can
thus be used to showcase the library's functionality. The \texttt{utils} subpackage (see
\cref{sec:pack-utils}) contains miscellaneous utility classes and
...
\subsection{The \texttt{ikkuna.export} subpackage}\label{sec:pack-export}}

The \texttt{ikkuna.export} subpackage contains the core part
...
\begin{table}
    ...
    \begin{tabularx}{\textwidth}{lX}
        \toprule
        Submodule           & Function\tabularnewline
        \midrule
...
\subsubsection*{The \texttt{ikkuna.export.export} submodule}
...
\cref{fig:pubsub}, the \texttt{ikkuna.export.Exporter} class
...
during registration of the module or generated automatically. The name is necessary
...
\begin{itemize}
    \item
        ...
        the phases\footnote{There are two built-in layers this applies to. One
            ...
            single data point entering the layer, as could be the case during
    ...
    \item
        ...
        \lstinline+forward()+ method, but want the following messages emitted by
        the \lstinline+Exporter+ to carry a specific tag, so that subscribers
        listening to the default tag are not notified.
    \item
    ...
    \item
        ...
        alternatively by identifying the last module in the network and
        attaching specific callbacks only to it, but is
        easier to do by patching the loss function set with
        \lstinline+set_loss()+, since the loss function will always be evaulated
...
for the current epoch or training step applying to the entire network. In that case, they
appear only once per epoch or training step, not once per layer. The meta-messages can
...
error checking to ensure consistency of the contained messages (e.g.~only
messages from the same time step are allowed).
...
Nevertheless, they can be used by the reader to start using the library.
...
All experimental code used for \cref{ch:experiments} is located in the \texttt{experiments} subfolder
accompanying this thesis. It can be used for reproducing all experiments
...
the Python package index\footnote{\url{https://pypi.org/}} by use of the package manager
\texttt{pip}:
...
\begin{quote}
    ...
    speed.\footnote{\url{https://www.peltarion.com/about}, accessed 17/01/19}
\end{quote}
