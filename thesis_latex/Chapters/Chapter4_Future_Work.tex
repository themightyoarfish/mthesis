...
The \texttt{ikkuna} library was employed in \cref{sec:experiments} to simplify
...
The experiments on learning rate optimisation (\cref{sec:I}) showed that a
static target ratio between updates and weights may not be the best approach.
Therefore, the experiments should be extended with an exploration of whether a
moving target ratio leads to faster convergence, before the idea is entirely
abandoned.
